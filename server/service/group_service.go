...
func CreateGroup(message *userConnections.Message, chanOut chan *userConnections.Message) {
	var createGroup dbinterfaces.GroupManager = &dbservice.Group{}
	switch message.GroupType {
	// groupType == 1 means privat message
	case 1:
		ok := createGroup.CreateGroup(message.GroupName, message.GroupOwner, message.GroupType)
		if ok {
			for _, user := range message.GroupMember {
				createGroup.AddGroupMember(user, message.GroupName, "")
			}
			message.Status = ok
		}
		message.Status = ok
	...
	case 2:
		ok := createGroup.CreateGroup(message.GroupName, message.GroupOwner, message.GroupType)
		if ok {
			createGroup.AddGroupMember(message.GroupOwner, message.GroupName, "")
			message.Status = ok
		}
		message.Status = ok
	}
